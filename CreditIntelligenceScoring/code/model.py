...
from sklearn.model_selection import KFold, RepeatedKFold    # 为了充分的利用训练样本及提高模型的准确率，经常需要应用交叉验证的方法
...
f = open('../result/result.txt', 'w')   # 保留每一个模型的正确率


def lgb1_model(num_model_seed, x_train, y_train, x_test, name):
    predictions_lgb = np.zeros(len(x_test)) # 声明预测数据
    oof = np.zeros(len(x_train))
    seeds = [2022, 1011, 2022 * 2 + 1024, 4096, 2048, 1024]
    for model_seed in range(num_model_seed):
        ...
        for fold_, (trn_idx, val_idx) in enumerate(folds.split(x_train, y_train)):
            ...
            clf.save_model('../result/models/lgb1_model_{}.txt'.format(fold_+1))
...
def lgb2_model(num_model_seed, x_train, y_train, x_test, name):
    ...
    for model_seed in range(num_model_seed):
        ...
        for fold_, (trn_idx, val_idx) in enumerate(folds.split(x_train, y_train)):
            ...
            clf = lgb.train(param, trn_data, num_round, valid_sets=[trn_data, val_data], verbose_eval=1000,
                            early_stopping_rounds=500)
            clf.save_model('../result/models/lgb2_model_{}.txt'.format(fold_ + 1))
            oof_lgb[val_idx] = clf.predict(x_train[val_idx], num_iteration=clf.best_iteration)
            predictions_lgb += clf.predict(x_test, num_iteration=clf.best_iteration) / folds.n_splits / num_model_seed
        oof += oof_lgb / num_model_seed
        MAE = mean_absolute_error(oof_lgb, y_train)
...
def xgb_model(num_model_seed, x_train, y_train, x_test, name):
    ...
    seeds = [2022, 1011, 4096, 2022 * 2 + 1024, 2048, 1024]
    for seed in range(num_model_seed):
        xgb_params = {'eta': 0.004,
                      'max_depth': 6,
                      'subsample': 0.5,
        ...
        for fold_, (trn_idx, val_idx) in enumerate(folds.split(x_train, y_train)):
            ...
            clf.save_model('../result/models/xgb_model_{}.txt'.format(fold_ + 1))
...
def cat_model(num_model_seed, x_train, y_train, x_test, name):
    ...
    for seed in range(num_model_seed):
        ...
        for fold_, (trn_idx, val_idx) in enumerate(folds.split(x_train, y_train)):
            ...
            clf.save_model('../result/models/cat_model_{}.txt'.format(fold_ + 1))
...
def lgb3_model(num_model_seed, x_train, y_train, x_test, name):
    ...
    for model_seed in range(num_model_seed):
        ...
        for fold_, (trn_idx, val_idx) in enumerate(folds.split(x_train, y_train)):
            ...
            clf = lgb.train(param, trn_data, num_round, valid_sets=[trn_data, val_data], verbose_eval=1000,
                            early_stopping_rounds=500)
            clf.save_model('../result/models/lgb3_model_{}.txt'.format(fold_ + 1))
            oof_lgb[val_idx] = clf.predict(x_train[val_idx], num_iteration=clf.best_iteration)
            predictions_lgb += clf.predict(x_test, num_iteration=clf.best_iteration) / folds.n_splits / num_model_seed
...
def lgb4_model(num_model_seed, x_train, y_train, x_test, name):
    ...
    for model_seed in range(num_model_seed):
        ...
        for fold_, (trn_idx, val_idx) in enumerate(folds.split(x_train, y_train)):
            ...
            clf = lgb.train(param, trn_data, num_round, valid_sets=[trn_data, val_data], verbose_eval=1000,
                            early_stopping_rounds=500)
            clf.save_model('../result/models/lgb4_model_{}.txt'.format(fold_ + 1))
            oof_lgb[val_idx] = clf.predict(x_train[val_idx], num_iteration=clf.best_iteration)
            predictions_lgb += clf.predict(x_test, num_iteration=clf.best_iteration) / folds.n_splits / num_model_seed
