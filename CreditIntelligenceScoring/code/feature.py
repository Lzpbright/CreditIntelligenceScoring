...
# 将训练集和测试集再0轴组合, 便于共同处理，也便于分开
data = pd.concat([train, test], axis=0, ignore_index=True)
data = data.fillna(0)  # 去除nan数据
...
def feature_count(data, features=[]):
    if len(set(features)) != len(features):  # 判断特侦值是否有重复，有的话就提示有相同的特征
        print('equal feature !!!!')
        return data
    new_feature = 'count'
    for i in features:
...
'''
接下来的6个函数是为了给6个模型进行训练进行的数据的特征提取，从而降低各个特征之间的相关性，
获取每个模型数据所需要的特征，同时将测试集和训练集进行合并的目的就是为了统一处理
'''
...
def feature1(data):
    ...
    data.drop(columns=['是否大学生客户', '是否黑名单客户', '当月是否到过福州山姆会员店', '当月是否逛过福州仓山万达',
...
def feature5(data):
    ...
    data['shopping_level'] = data['当月是否逛过福州仓山万达'] + data['当月是否到过福州山姆会员店'] + data[
        '当月是否看电影'] + data['当月是否景点游览'] + data[
                                 '当月是否体育场馆消费']
    data['当月网购类应用使用次数/all'] = \
        data['当月网购类应用使用次数'] / (data['当月视频播放类应用使用次数'] + data['当月物流快递类应用使用次数'] + \
                                          data['当月旅游资讯类应用使用次数'] + data['当月飞机类应用使用次数'] + data[
                                              '当月火车类应用使用次数'] + \
                                          data['当月金融理财类应用使用总次数'])

    data['当月交通类应用使用次数/all'] = \
        (data['当月飞机类应用使用次数'] + data['当月火车类应用使用次数']) / (
                data['当月视频播放类应用使用次数'] + data['当月物流快递类应用使用次数'] + \
                data['当月网购类应用使用次数'] + data['当月旅游资讯类应用使用次数'] +
                data['当月金融理财类应用使用总次数'])
    data.drop(columns=['是否大学生客户', '是否黑名单客户', '当月是否到过福州山姆会员店', '当月是否逛过福州仓山万达',
                       '是否经常逛商场的人'], inplace=True)
